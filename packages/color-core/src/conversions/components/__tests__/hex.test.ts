...
describe('Hex <-> RGB Conversions', () => {
  describe('rgbToHex', () => {
    ...
    it('should handle alpha channel correctly', () => {
      ...
      expect(rgbToHex({ r: 0, g: 255, b: 0, a: 1 }, true)).toBe('#00ff00ff')
      expect(rgbToHex({ r: 0, g: 0, b: 255, a: 0 }, true)).toBe('#0000ff00')
    })
  ...
  describe('Test for [issue #22](https://github.com/iamlite/color-core/issues/22)', () => {
    it('should handle hex to rgb with alpha to hex', () => {
      expect(new Color('#4aa444').setAlpha(0.5).toHex()).toBe('#4aa44480')
    })
  })
})
