...
describe('Color Class', () => {
  describe('Constructor', () => {
    ...
    it('should create a color from LAB object', () => {
      const color = new Color({ l: 53.23, a: 80.11, b: 67.22 });
      expect(color.toRgb()).toEqual({ r: 255, g: 0, b: 0 });
    });
    ...
    it('should create a color from YUV object', () => {
      const color = new Color({ y: 0.299, u: -0.14713, v: 0.615 });
      expect(color.toRgb()).toEqual({ r: 255, g: 0, b: 0 });
    });
  ...
  describe('Conversion Methods', () => {
    ...
    it('should convert to LCH', () => {
      ...
      expect(lch.h).toBeCloseTo(40, 1);
    });

    it('should convert to XYZ', () => {
      const xyz = color.toXyz();
      expect(xyz.x).toBeCloseTo(0.4124, 4);
      expect(xyz.y).toBeCloseTo(0.2126, 4);
      expect(xyz.z).toBeCloseTo(0.0193, 4);
    });

    it('should convert to YUV', () => {
      const yuv = color.toYuv();
      expect(yuv.y).toBeCloseTo(0.299, 3);
      expect(yuv.u).toBeCloseTo(-0.14713, 3);
      expect(yuv.v).toBeCloseTo(0.615, 3);
    });
